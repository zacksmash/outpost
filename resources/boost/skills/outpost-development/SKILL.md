...
- At most `outpost.max_concurrent_provisions` instances (default 3) boot and provision at once per project. Launch as many creations, recreations, upgrades, or starts as you like in parallel: extra runs print `Waiting for a provisioning slot` and proceed automatically when one frees, so never add your own pacing, retries, or timeouts around a waiting run. A killed provision releases its slot immediately. The limit does not span projects — when fanning out across several repositories at once, lower it in each.
...
- `outpost:exec` passes tokens without a shell, streams output, preserves the inner exit code, and runs as the host-ID-mapped non-root user. Use `--root` only for intentional elevation. There is no timeout by default; pass `--timeout=<seconds>` to fail fast on commands that may hang. The deadline is enforced inside the instance, killing the real command, and exit code `124` means it expired. When the failure message mentions the exec session not responding, the session is wedged — `outpost:recover <name> --force` is the remedy. Do not use `--timeout` around dependency installs or builds, which legitimately run long.
...
Hook failure preserves the instance and fails the lifecycle operation. Hooks always run as the non-root application user; there is no privileged hook mode. The base image supplies Playwright's Ubuntu Chromium dependencies but not a browser binary, so a setup hook may safely download the project's matching browser with `npx playwright install chromium`; Passport keys likewise need a `['@php', 'artisan', 'passport:keys', '--force']` setup hook. `outpost:doctor` warns with the exact hook to add when it detects Playwright or Passport without one — configure the hook rather than running these commands manually per instance. Teardown hooks run only when the manifest is ready and the container is running; stopped, missing, and incomplete instances skip them. `outpost:remove --forget` bypasses even malformed hook configuration when runtime recovery is impossible.
