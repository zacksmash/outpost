...
- do not edit an external path repository through the worktree's `vendor/` symlink or its ignored host bridge unless that external source is explicitly part of the task; the container mount is read-only, but host symlinks cannot enforce that restriction
...
- The app installs a sibling local package via a Composer path repository such as `../outpost`: Outpost resolves it from the primary checkout, mounts it read-only where the URL resolves from `/app` (here `/outpost`), creates an ignored host bridge so Composer's vendor symlink also resolves for host tools, and keeps its own runtime files under `/etc/outpost`; pass `--mount-path-repos` in scripts that must not prompt.
