...
it('builds the base image from the package stubs', function () {
    ...
    Process::assertRan(function (PendingProcess $process) {
        ...
        return array_slice($process->command, 0, 6) === [
            'container', 'build', '--dns', '1.1.1.1', '--tag', 'ghcr.io/zacksmash/outpost:0.4.0',
...
it('keeps an existing image when the rebuild is declined', function () {
    Process::fake([
        processPattern('container', 'image', 'inspect', 'ghcr.io/zacksmash/outpost:0.4.0') => Process::result('[{"reference":"ghcr.io/zacksmash/outpost:0.4.0"}]'),
    ]);

    $this->artisan('outpost:build')
        ->expectsConfirmation('The [ghcr.io/zacksmash/outpost:0.4.0] image already exists. Rebuild it?', 'no')
...
it('rebuilds an existing image when confirmed', function () {
    ...
    $this->artisan('outpost:build')
        ->expectsConfirmation('The [ghcr.io/zacksmash/outpost:0.4.0] image already exists. Rebuild it?', 'yes')
        ->assertSuccessful();
