...
it('keeps an existing image when refresh is declined', function () {
    ...
    $this->artisan('outpost:pull')
        ->expectsConfirmation('The [ghcr.io/zacksmash/outpost:0.1.1] image already exists. Pull it again?', 'no')
...
it('surfaces registry failures with the local build fallback', function () {
    Process::fake([
        processPattern('container', 'image', 'inspect', 'ghcr.io/zacksmash/outpost:0.1.1') => Process::result('', 'not found', 1),
        processPattern('container', 'image', 'pull', 'ghcr.io/zacksmash/outpost:0.1.1') => Process::result('', 'registry unavailable', 1),
    ]);
