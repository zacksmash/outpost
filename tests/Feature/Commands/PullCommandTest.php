...
it('pulls the configured image when it is missing', function () {
    Process::fake([
        ...
        processPattern('container', 'image', 'pull', 'ghcr.io/zacksmash/outpost:0.5.5') => Process::result('pulled'),
    ]);
...
it('keeps an existing image when refresh is declined', function () {
    Process::fake([
        processPattern('container', 'image', 'inspect', 'ghcr.io/zacksmash/outpost:0.5.5') => Process::result('[{}]'),
    ]);

    $this->artisan('outpost:pull')
        ->expectsConfirmation('The [ghcr.io/zacksmash/outpost:0.5.5] image already exists. Pull it again?', 'no')
...
it('surfaces registry failures with the local build fallback', function () {
    Process::fake([
        processPattern('container', 'image', 'inspect', 'ghcr.io/zacksmash/outpost:0.5.5') => Process::result('', 'not found', 1),
        processPattern('container', 'image', 'pull', 'ghcr.io/zacksmash/outpost:0.5.5') => Process::result('', 'registry unavailable', 1),
    ]);
