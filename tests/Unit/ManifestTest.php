...
it('detects image reference and digest upgrades while keeping legacy state unknown', function () {
    ...
    expect(fakeManifest()->imageOutdated('ghcr.io/zacksmash/outpost:0.8.2', $current))->toBeFalse()
        ->and(fakeManifest()->imageOutdated('ghcr.io/zacksmash/outpost:0.2.2', $current))->toBeTrue()
        ->and(fakeManifest()->imageOutdated('ghcr.io/zacksmash/outpost:0.8.2', $new))->toBeTrue()
        ->and(fakeManifest()->imageOutdated('ghcr.io/zacksmash/outpost:0.8.2', null))->toBeNull()
        ->and(fakeManifest(image: null, imageDigest: null)
            ->imageOutdated('ghcr.io/zacksmash/outpost:0.8.2', $current))->toBeNull();
});
